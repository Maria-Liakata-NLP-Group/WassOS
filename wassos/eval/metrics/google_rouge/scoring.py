...
class BootstrapAggregator(object):
    ...
    def aggregate(self):
        ...
        for score_type, scores in six.iteritems(self._scores):
            # Stack scores into a 3-d matrix of (sample, measure).
    ...
    def _bootstrap_resample(self, matrix):
        ...
        Args:
          matrix: A 3-d matrix of (sample, measure).
        Returns:
          A 3-d matrix of (bounds, measure). There are three bounds: low (row 0),
          mid (row 1) and high (row 3). Mid is always the mean, while low and high
          bounds are specified by self._confidence_interval (which defaults to 0.95
          meaning it will return the 3.5th and 97.5th percentiles for a 95%
