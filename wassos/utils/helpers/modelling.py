...
def group_att_over_input(inp_att_vals, inp_att_mask, att_indxs, att_indxs_mask,
    ...
    # the size of inp_att_vals is [batch_size, max_rev_pergroup, sentence_length, dim]]
    ...
    if inp_att_keys is not None:
        ...
        return inp_att_keys,inp_att_vals, inp_att_mask
    else:
        # return att_vals, att_mask
        return inp_att_vals, inp_att_mask



def group_att_over_input_for_decode(inp_att_vals, inp_att_mask, att_indxs, att_indxs_mask,
                         inp_att_keys=None):
    """
    Returns properly concatenated and shaped attention keys/values/mask over
    input (e.g. hidden states and embds).
