...
from wassos.data_pipelines.steps import GroupFileShuffler, SummRevIndxsCreator, \
    RevMapper
from wassos.utils.fields import InpDataF, ModelF
from csv import QUOTE_NONE


def assemble_train_pipeline(word_vocab,tag_vocab, max_groups_per_batch=1,
    ...
    assert START in tag_vocab
    assert END in tag_vocab
    ...
    fname_renamer = FieldRenamer({InpDataF.REV_TEXT: ModelF.REV,
                                  InpDataF.REV_TAG: ModelF.REV_TAG,
    ...
    tokenizer = TokenProcessor(fnames=[ModelF.REV, ModelF.REV_TAG])
    vocab_mapper = VocabMapper({ModelF.REV: word_vocab,
                                ModelF.REV_TAG: tag_vocab
                                })

    seq_wrapper = SeqWrapper(fname=[ModelF.REV, ModelF.REV_TAG],
                             start_el=word_vocab[START].id,
                             end_el=word_vocab[END].id)

    seq_len_computer = SeqLenComputer([ModelF.REV, ModelF.REV_TAG], [ModelF.REV_LEN, ModelF.REV_TAG_LEN])

    sorter = ChunkSorter(ModelF.REV_LEN)
    # sorter_tag = ChunkSorter(ModelF.REV_TAG_LEN)

    padder = Padder(fname=ModelF.REV,
                    new_mask_fname=ModelF.REV_MASK,
                    pad_symbol=word_vocab[PAD].id, padding_mode='right')
    padder_tag = Padder(fname=ModelF.REV_TAG,
                    new_mask_fname=ModelF.REV_TAG_MASK,
                    pad_symbol=word_vocab[PAD].id, padding_mode='right')

    summ_rev_indxs_creator = SummRevIndxsCreator(group_id_fname=ModelF.GROUP_ID,
                                                 category_fname=ModelF.CAT)

    # 这个方法对于tag_sent来说可以不用
    ...
    # Adds a new processing component to the end of the chain of processing steps.
    ...
    # pipeline.add_step(sorter_tag)
    pipeline.add_step(padder)
    pipeline.add_step(padder_tag)
