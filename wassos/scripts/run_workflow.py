...
from wassos.data_pipelines.assemblers import assemble_train_pipeline, \
    assemble_eval_pipeline, assemble_vocab_pipeline
from wassos.data_pipelines.assemblers.infer_pipeline import assemble_infer_pipeline
...
from wassos.modelling.interfaces import IDevWassOS as IDev, IWassOS as IModel
from wassos.modelling import WassOS as Model
from mltoolkit.mlmo.generation import Beamer
from wassos.utils.fields import InpDataF
from torch.nn.init import xavier_uniform_, normal_
from mltoolkit.mlutils.helpers.formatting.general import format_big_box
from wassos.utils.helpers.run import gen_seqs, summ_eval, gen_summs
from wassos.utils.helpers.io import get_rev_number
...
from wassos.utils.tools import SeqPostProcessor
from wassos.utils.constants import VOCAB_DEFAULT_SYMBOLS
...
# =====================change================================
tag_pipline = assemble_vocab_pipeline(text_fname=InpDataF.REV_TAG)
tag_vocab = Vocabulary(tag_pipline, name_prefix='tag')
...
tag_vocab.load_or_create(run_hp.tag_vocab_fp,
                          data_source=vocab_data_source,
                          max_size=model_hp.ext_tag_size, sep=' ',
                          data_fnames=InpDataF.REV_TAG)


word_vocab.write(comb_paths(run_hp.output_path, "vocab.txt"), sep=' ')
tag_vocab.write(comb_paths(run_hp.output_path, "tag.txt"), sep=' ')

train_pipeline = assemble_train_pipeline(word_vocab,tag_vocab,
                                         ...
                                         seed=None, workers=1)
val_pipeline = assemble_train_pipeline(word_vocab,tag_vocab,
...
def yaml_to_dict(yamml_in_file):
    with open(yamml_in_file, 'r', encoding='utf-8') as f:
        arg_dicts = yaml.load(f.read())

    return arg_dicts

def dic_to_args(dicts):
    return Namespace(**dicts)

args = dic_to_args(yaml_to_dict('syntax-vae.yaml')['model_configs'])


# model = Model(batch_size = run_hp.val_max_groups_per_batch, max_rev_per_group = run_hp.max_rev_per_group, **model_hp.to_dict())
model = Model(args=args, vocab=word_vocab, tag_vocab=tag_vocab, max_rev_per_group = run_hp.max_rev_per_group, **model_hp.to_dict())
