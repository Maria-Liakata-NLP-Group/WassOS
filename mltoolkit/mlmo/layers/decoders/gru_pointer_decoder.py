...
class GruPointerDecoder(Module):
    ...
    At each time step it runs the attention mechanism and computes the context
    ...
    def __init__(self, input_dim, contxt_dim, hidden_dim, att_module,
                 pointer_gen_module,transformer_decoder,  logit_fun, word_padding_idx=0, cat_contx_to_inp=True,
        ...
        # self.linear = Linear(800, 768)
        # self.z_linear = Linear(600, 768)
        # self.LayerNorm = T.nn.LayerNorm(768, eps=1e-6)





    def forward(self, embds, mask, init_hidden, att_keys=None, att_values=None, att_mask=None,
                att_word_ids=None, init_cont=None, extra_feat=None, review_word_id=None, mode=None,
                **att_kwargs):
        """

        review_word_id=None,  init_cont=None, extra_feat=None,
        ...
        if att_values is not None and att_mask is not None:
        ...
        # src = review_word_id.transpose(0, 1).contiguous()
        src = review_word_id
        tgt = None
        ...
        for indx in range(out_seq_len):
            ...
            if index < out_seq_len:
                index += 1

            if tgt is not None:
                tgt = review_word_id[:, :index]


            gru_inp = _x if extra_feat is None \
                else T.cat((_x, extra_feat[:, indx]), dim=-1)

            src_features = extra_feat[:, :index]

            if mode is not None:
                 old_hidden = self.step(_x, _m,
                                          ...
                                          tgt=tgt,
                                          src=src,
                                          # src_features=memory_bank,
                                          src_features=src_features,
                                          mode=mode,
                                          **att_kwargs)
                 logit = self.logit_fun(old_hidden)
                 logits.append(logit)


            else:

                hidden, cont, att_wts, old_hidden = self.step(_x, _m,
                                                  prev_hidden=prev_hidden,
                                                  prev_cont=prev_cont,
                                                  att_keys=att_keys,
                                                  att_values=att_values,
                ...
                if self.pass_extra_feat_to_pg and extra_feat is not None:
                    ...
                    # print(self.pass_extra_feat_to_pg,"pass_extra_feat_to_pg=========")
                else:
                    pgn_inp = _x
    ...
    def step(self, emb, m, prev_hidden, prev_cont, att_keys, att_values,
             att_mask,tgt=None, mode=None, src=None, src_features=None,  **att_kwargs):
        """
        Performs a decoding step by running the GRU cell.

        tgt_emb, tgt=None, src=None,
        ...
        if src is not None:
            src_mask = get_pad_mask(src, self.word_padding_idx)
        if mode is None:
            hidden, *_ = self.decoder(src_features, trg_mask, src_mask)
            etr_inp = hidden[:, -1, :]
        else:
            etr_inp = src_features[:, -1]

        inp = T.cat((inp, etr_inp), dim=-1)
        ...
        if mode is not None:
            ...
            return new_hidden, new_cont, att_wts, curr_hidden
